...
def psnr(output, target, valid_mask=None):
    ...
    mse = torch.sum(mse, (-1, -2)) / n
    max_pixel_value = 1.0  # 假设像素值范围在[0, 1]之间，如果是[0, 255]，需要调整
...
def ssim(output, target, valid_mask=None):
    ...
    ssim_module.to(output.device)
    if valid_mask is not None:
        output = output * valid_mask
        target = target * valid_mask

    # 如果图像是单张的，则将其扩展为 batch 格式
...
def Lpips(output, target, model='alex', valid_mask=None):

    lpips_model.to(output.device)
    # 将图像格式转换为LPIPS模型所需格式（一般为[batch_size, 3, H, W]）
    if len(output.shape) == 3:
        output = output.unsqueeze(0)
        target = target.unsqueeze(0)

    lpips_value = lpips_model(output, target)
